...
class LinearEquationTests(unittest.TestCase):
    ...
    def test_4_unknowns(self):
        ...
        def e1(genes):
            x, y, z, a = genes
            return Fraction(1, 15) * x   - 2 * - 15* z - Fraction(4,5)*a - 3

        def e2(genes):
            x, y, z, a = genes
            return Fraction(5, 2) * x - Fraction(9, 4) * y + 12 * z - a- 17

        def e3(genes):
            x, y, z, a = genes
            return -13 * x - Fraction(3, 10) * y - 6 * z - Fraction(2, 5) * a- 17

        def e4(genes):
            x, y, z, a = genes
            return Fraction(1, 2) * x + 2 * y+Fraction(7, 4) * z + Fraction(4, 3) * a + 9
